    ...
    if contexto == "[Administração Geral]":
        ...
        if menu == "👥 Funcionários":
            ...
            if not camps: st.info("Nenhuma campanha ativa.")
                ...
                # ADICIONADA A NOVA ABA DO RELATÓRIO CLASSIFICATÓRIO
                tab_exec, tab_classif, tab_estat, tab_ger, tab_bruto = st.tabs(["📊 Dashboard Gráfico", "📋 Relatório Classificatório", "📈 Estatísticas", "⚙️ Gerenciar", "📥 Dados Brutos"])
                ...
                # --- MOTOR CENTRAL DE CÁLCULO (Disponível para todas as abas) ---
                if not df_b.empty:
                ...
                if df_b.empty: st.warning("Aguardando primeiras respostas...")
                    ...
                    # --- NOVA ABA CLASSIFICATÓRIA (CÉREBRO DA MATEMÁTICA E TEXTOS) ---
                    with tab_classif:
                        st.markdown("""
                        <style>
                        @media print {
                            /* Margem reduzida para 10mm e Zoom reduzido para 85% para caber mais itens por página */
                        ...
                        # Dicionário de Fatores de Risco e Ações (Cobrimos os nomes duplos para segurança)
                        DICT_FATORES = {
                        ...
                        # Dicionário de Observações Dinâmicas para Tabela 2
                        OBS_MACRO = {
                        ...
                        # Gerador de HTML para Tabelas Perfeitas na Impressão (VERSÃO COMPACTA)
                        def build_html_table(df, headers, widths):
                            # Reduzido font-size para 10px e margin-bottom para 10px
                            ...
                            for i, h in enumerate(headers):
                                # Padding reduzido para 4px para a tabela ficar mais fina
                                html += f"<th style='padding: 4px; border: 1px solid #ddd; width: {widths[i]}; text-align: left;'>{h}</th>"
                            html += "</tr></thead><tbody>"
                        ...
                        df_tabela1_print = df_tabela1.drop(columns=['macro_nome'])

                        # --- CÁLCULO DA TABELA 2 ---
                        df_macro_classif = df_tabela1.groupby('macro_nome')['RESULTADO'].mean().reset_index()
                        tabela2_data = []
                        ...
                        df_tabela2_print = df_tabela2.drop(columns=['OBSERVAÇÃO'])

                        # --- LÓGICA DO LAUDO CONCLUSIVO ---
                        n_fav = len(df_tabela2[df_tabela2['CLASSIFICAÇÃO'] == 'FAVORÁVEL'])
                        n_mod = len(df_tabela2[df_tabela2['CLASSIFICAÇÃO'] == 'MODERADO'])
                        n_ris = len(df_tabela2[df_tabela2['CLASSIFICAÇÃO'] == 'RISCO'])

                        if n_ris > 0: max_risk = "RISCO"
                        elif n_mod > 0: max_risk = "MODERADO"
                        else: max_risk = "FAVORÁVEL"
                        ...
                        # --- RENDERIZAÇÃO DO RELATÓRIO NA TELA ---
                        st.markdown(f"""
                        <div style='text-align: center; margin-bottom: 30px; line-height: 1.2;'>
                            <span style='color: black; font-size: 28px; font-weight: bold;'>Relatório Diagnóstico de Risco Psicossocial</span><br>
                            <span style='color: black; font-size: 20px; font-weight: bold;'>{contexto}</span><br>
                            <span style='color: #666; font-size: 16px;'>Campanha: {c_obj.nome_campanha} | Vigência: {c_obj.data_inicio}</span>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        # --- TEXTOS FIXOS E TABELAS INFORMATIVAS ---
                        st.markdown("""
                        ...
                        <table style='width: 100%; border-collapse: collapse; margin-bottom: 10px; color: black; font-size: 10px; page-break-inside: avoid;'>
                            <thead>
                                <tr style='background-color: #1560bd; color: white;'>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left;'>FAIXA (%)</th>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left;'>INTERPRETAÇÃO</th>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left;'>RISCO</th>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left;'>RECOMENDAÇÕES</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>0 a 49.99%</td><td style='padding: 4px; border: 1px solid #ddd;'>Situação favorável</td><td style='padding: 4px; border: 1px solid #ddd; color: #16a34a; font-weight: bold;'>FAVORÁVEL</td><td style='padding: 4px; border: 1px solid #ddd;'>Monitorar</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>50 a 74.99%</td><td style='padding: 4px; border: 1px solid #ddd;'>Situação intermediária</td><td style='padding: 4px; border: 1px solid #ddd; color: #ca8a04; font-weight: bold;'>MODERADO</td><td style='padding: 4px; border: 1px solid #ddd;'>Planejar ações corretivas</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>75 a 100%</td><td style='padding: 4px; border: 1px solid #ddd;'>Situação crítica</td><td style='padding: 4px; border: 1px solid #ddd; color: #dc2626; font-weight: bold;'>RISCO</td><td style='padding: 4px; border: 1px solid #ddd;'>Intervenção imediata</td></tr>
                            </tbody>
                        </table>
                        ...
                        <table style='width: 100%; border-collapse: collapse; margin-bottom: 10px; color: black; font-size: 10px; page-break-inside: avoid;'>
                            <thead>
                                <tr style='background-color: #1560bd; color: white;'>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left; width: 25%;'>AÇÃO</th>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left; width: 55%;'>DESCRIÇÃO</th>
                                    <th style='padding: 4px; border: 1px solid #ddd; text-align: left; width: 20%;'>CRONOGRAMA</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>Acompanhamento psicológico</td><td style='padding: 4px; border: 1px solid #ddd;'>Criação de espaços de escuta, como rodas de conversa e atendimentos breves no ambiente laboral, voltados ao acolhimento.</td><td style='padding: 4px; border: 1px solid #ddd;'>Mensal, por demanda</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>Programas de sensibilização</td><td style='padding: 4px; border: 1px solid #ddd;'>Oficinas e palestras sobre saúde mental no trabalho, autocuidado, gestão emocional e prevenção de burnout.</td><td style='padding: 4px; border: 1px solid #ddd;'>Mensal, temas adaptados</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>Assessoria em gestão participativa</td><td style='padding: 4px; border: 1px solid #ddd;'>Apoio à liderança na implementação de práticas transparentes, colaborativas e justas.</td><td style='padding: 4px; border: 1px solid #ddd;'>Bimestral + follow-up</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>Reestruturação organizacional</td><td style='padding: 4px; border: 1px solid #ddd;'>Mediação entre trabalhadores e gestão para redimensionar tarefas e resgatar o propósito nas atividades.</td><td style='padding: 4px; border: 1px solid #ddd;'>90 dias</td></tr>
                                <tr><td style='padding: 4px; border: 1px solid #ddd;'>Protocolo de acompanhamento</td><td style='padding: 4px; border: 1px solid #ddd;'>Revisão periódica das condições de trabalho e escuta ativa para validação de medidas aplicadas.</td><td style='padding: 4px; border: 1px solid #ddd;'>60 dias</td></tr>
                            </tbody>
                        </table>
                        ...
                        # Imprime o texto de laudo gerado pelo Cérebro Matemático
                        ...
                        st.markdown(f"""
                        <div style='margin-top: 60px; text-align: center; color: black; page-break-inside: avoid;'>
                            <p>____________________________________________________________________</p>
                            <p style='margin: 0; font-weight: bold; font-size: 14px;'>Abel Luiz de Oliveira - Médico do Trabalho</p>
                            <p style='margin: 0; font-size: 12px;'>CRM/MG 10.419</p>
                            <p style='margin-top: 10px; font-size: 12px;'>Data: {datetime.now().strftime('%d/%m/%Y')}</p>
                        </div>
                        """, unsafe_allow_html=True)
