    ...
    if contexto == "[Administração Geral]":
        ...
        if menu == "👥 Funcionários":
            ...
            if not camps: st.info("Nenhuma campanha ativa.")
                ...
                # ADICIONADA A NOVA ABA "📖 Metodologia"
                tab_exec, tab_classif, tab_estat, tab_ger, tab_bruto, tab_metodo = st.tabs(["📊 Dashboard Gráfico", "📋 Relatório Classificatório", "📈 Estatísticas", "⚙️ Gerenciar", "📥 Dados Brutos", "📖 Metodologia"])
                ...
                df_b = pd.DataFrame(res, columns=colunas_df) if res else pd.DataFrame(columns=colunas_df)
                df_raw = df_b.copy()

                # --- MOTOR CENTRAL DE CÁLCULO (Padrão Acadêmico 0 a 100%) ---
                if not df_b.empty:
                ...
                with tab_ger:
                    ...
                    # --- NOVA FUNÇÃO DE IMPORTAÇÃO E EXPORTAÇÃO ANÔNIMA ---
                    st.divider()
                    ...
                    if perguntas_campanha:
                        ...
                        colunas_matriz = ['Identificador (Opcional)'] + [f"{p.id} - {p.enunciado}" for p in perguntas_campanha]
                        df_modelo = pd.DataFrame(columns=colunas_matriz)
                        # Adiciona umas 3 linhas vazias para o usuário entender o formato
                        ...
                        if todas_sessoes:
                            ...
                            df_export = df_export[colunas_matriz] # Ordena as colunas
                        ...
                        if up_respostas:
                            ...
                            if st.button("Confirmar Importação de Respostas", type="primary"):
                                sucesso_count = 0
                                cols_perguntas = [c for c in df_up_resp.columns if ' - ' in str(c)]
                                
                                for index, row in df_up_resp.iterrows():
                                    # Verifica se a linha tem pelo menos uma resposta válida antes de criar usuário fantasma
                                    tem_resposta = False
                                    ...
                                    # Cria funcionário "Fantasma/Anônimo"
                                    ...
                                    # Gera um CPF fake invisível garantido ser único
                                    ...
                                    db.add(func_anonimo)
                                    db.flush() # Para pegar o ID no banco
                ...
                if df_b.empty: 
                    ...
                    with tab_classif:
                        ...
                        # Dicionário de Fatores de Risco e Ações
                        DICT_FATORES = {
                        ...
                        # Gerador de HTML para Tabelas Perfeitas na Impressão (COM TEXTOS CENTRALIZADOS NAS COLUNAS ESPECÍFICAS)
                        def build_html_table(df, headers, widths):
                            ...
                            for i, h in enumerate(headers):
                                # Identifica se a coluna alvo precisa ser centralizada
                            ...
                            for _, row in df.iterrows():
                                ...
                                for i, col in enumerate(df.columns):
                                    ...
                                    # Aplica a mesma centralização nas células de dados
                        ...
                        # --- TEXTOS FIXOS E TABELAS INFORMATIVAS (COM CENTRALIZAÇÃO APLICADA) ---
                        st.markdown("""
                ...
                with tab_metodo:
                    ...
                    if st.button("🖨️ Imprimir Metodologia / Ata"):
                        script = f"<script>window.parent.print();</script><div style='display:none;'>{time.time()}</div>"
                        st.components.v1.html(script, height=0)

                    total_respostas_campanha = df_raw['CPF'].nunique() if not df_raw.empty else 0
                    ...
                    <div style='border: 2px solid #000; padding: 20px; margin-top: 20px;'>
                        <div style='text-align: center; margin-bottom: 15px;'>
                            <span style='font-size: 18px; font-weight: bold; text-decoration: underline;'>ATA DE TRANSPARÊNCIA E REPLICABILIDADE</span>
                        </div>
                        <p style='font-size: 12px; margin-bottom: 5px;'><b>Organização Avaliada:</b> {contexto}</p>
                        <p style='font-size: 12px; margin-bottom: 5px;'><b>Identificação da Campanha:</b> {c_obj.nome_campanha}</p>
                        <p style='font-size: 12px; margin-bottom: 5px;'><b>Data de Início da Avaliação:</b> {c_obj.data_inicio}</p>
                        <p style='font-size: 12px; margin-bottom: 5px;'><b>Trabalhadores Respondentes (Amostra):</b> {total_respostas_campanha}</p>
                        <p style='font-size: 12px; margin-bottom: 5px;'><b>Instrumento de Avaliação:</b> COPSOQ-II (Versão Média)</p>
                        <br>
                        <p style='font-size: 12px; text-align: justify;'>
                        Atestamos que os dados consolidados nos relatórios anexos refletem fielmente as respostas obtidas por meio do questionário estruturado, mantendo-se o sigilo absoluto e a confidencialidade dos dados individuais dos colaboradores, em consonância com as diretrizes éticas de Saúde Ocupacional e a LGPD.
                        </p>
                        <br><br><br>
                        <div style='text-align: center;'>
                            ...
                            <p style='margin: 0; font-weight: bold; font-size: 14px;'>Abel Luiz de Oliveira - Médico do Trabalho</p>
                            <p style='margin: 0; font-size: 12px;'>CRM/MG 10.419</p>
                            <p style='margin-top: 10px; font-size: 12px;'>Data da Emissão: {datetime.now().strftime('%d/%m/%Y')}</p>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
