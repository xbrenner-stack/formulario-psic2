...
# --- ÁREA ADMINISTRATIVA ---
def admin_portal():
    st.sidebar.title("🛡️ Acesso ao Sistema")
    if 'admin_logged_in' not in st.session_state:
        with st.sidebar.form("admin_login"):
            ...
            if st.form_submit_button("Acessar"):
                ...
                if user in usuarios and pw == str(usuarios[user].get("senha")):
                    st.session_state['admin_logged_in'] = True
                    st.session_state['user_role'] = usuarios[user].get("role", "avaliador")
                    st.session_state['nome_avaliador'] = usuarios[user].get("nome", "Avaliador")
                    st.rerun()
                else:
    ...
    if emp_id_selecionado is None:
        if st.session_state.get('user_role') == 'admin':
            ...
            menu = st.sidebar.radio("Navegação Global", ["🏢 Hub de Empresas", "🚪 Sair"])
        
        if menu == "🚪 Sair":
            st.session_state.pop('admin_logged_in', None)
            st.session_state.pop('user_role', None)
            st.session_state.pop('nome_avaliador', None)
            st.rerun()
        ...
        if st.session_state.get('user_role') == 'admin':
            opcoes_menu_cliente = ["👥 Funcionários", "📊 Campanhas e Resultados", "📋 Auditoria MTE", "🚪 Sair"]
        else:
            opcoes_menu_cliente = ["📋 Auditoria MTE", "🚪 Sair"]
            
        menu = st.sidebar.radio("Navegação do Cliente", opcoes_menu_cliente)
...
'''
            ...
            st.sidebar.info(f"👤 Avaliador logado: **{nome_pdf}**")
            modulo_mte.renderizar_auditoria_mte(empresa_selecionada.id, nome_pdf)
